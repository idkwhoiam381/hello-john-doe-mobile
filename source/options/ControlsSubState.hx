...
class ControlsSubState extends MusicBeatSubstate
{
	...
	override function update(elapsed:Float)
	{
		...
		if(!binding)
		{
			...
			if(controls.ACCEPT || FlxG.gamepads.anyJustPressed(START) || FlxG.gamepads.anyJustPressed(A))
			{
				if(options[curOptions[curSelected]][1] != defaultKey)
				{
					...
					bindingText2 = new ControlsText(0, 340, "Hold B to Cancel\nHold C to Delete", true);
		...
		else
		{
			...
			if(MusicBeatSubstate.virtualPad.buttonB.pressed || FlxG.keys.pressed.ESCAPE || FlxG.gamepads.anyPressed(B))
			{
				holdingEsc += elapsed;
				if(holdingEsc > 0.5)
				{
